...
class SubjectDetailsViewModel: SubjectDetailsViewModelRepresentable, CommentRepresentableProtocol {
    ...
    var teacherName: NSAttributedString {
        ...
        attributedString.addAttribute(
            .font,
            value: AppFont.font(type: .medium, size: 16),
            range: NSRange(location: head.count, length: tail.count)
        )
    ...
    var homeworkFileURLs: [String]? {
        self.lessonDetails?.homework?.files.compactMap { $0.file }
    }
    ...
    func submit(_: Int? = nil) async throws {
        guard !attachedFiles.isEmpty,
